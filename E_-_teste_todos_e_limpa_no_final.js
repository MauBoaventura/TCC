...
stringJson10 = JSON.parse(fs.readFileSync("./10-documents.json", "utf8"))
...
const insere10 = () => MongoClient.connect(url, function (err, db) {
  ...
  console.log("insere 10 levou " + duracao + "ms")
  db.close();
});
...
const insere100 = () => MongoClient.connect(url, function (err, db) {
  ...
  var duracao = Date.now() - antes;
  console.log("insere 100 levou " + duracao + "ms")
  db.close();
});
...
const insere1000 = () => MongoClient.connect(url, function (err, db) {
  ...
  var duracao = Date.now() - antes;
  console.log("insere 1000 levou " + duracao + "ms")
  db.close();
});
